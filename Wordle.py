...
def text_read(user_text: str, start: int = 1, end: float = inf, limit : float = inf, start_substring: str = '', end_substring: str = '', including : bool = False):
    line = 1
    text = ''
    cut_text = ''
    line_symb_count = 0
    for i in range(len(user_text)):
        m = user_text[i]
        text += m if m != '\n' and start <= line <= end else ''
        if m == '\n':
            text += '\n' if start <= line < end else ''
            line += 1
    text = text.strip()
    substring_starting_number = text.find(start_substring) + len(start_substring) - 1 if start_substring != '' else - 1
    substring_ending_number = text.find(end_substring) if end_substring != '' else len(text)
    if substring_starting_number - len(start_substring) + 1 == substring_ending_number:
        substring_ending_number = text[(substring_starting_number + 1)::].find(end_substring) + substring_starting_number + 1 if end_substring != '' else - 1
    if not including:
        for i in range(substring_starting_number, substring_ending_number):
            if line_symb_count <= limit:
                cut_text += text[i] if text[i] != '\n' else ''
            if text[i] == '\n':
                cut_text += text[i]
                line_symb_count = 0
            line_symb_count += 1
        cut_text = cut_text[1::].strip()
    elif including:
        for i in range(substring_starting_number - len(start_substring), substring_ending_number + len(end_substring)):
            if line_symb_count <= limit:
                cut_text += text[i] if text[i] != '\n' else ''
            if text[i] == '\n':
                cut_text += text[i]
